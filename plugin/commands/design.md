...
Read `.claude/sdlc.yml` for `language`, `task_management`, `team_key`. No decisions here — pass through. You do **not** need to read `phases` — the `phase-tuning` PreToolUse hook applies per-phase model/effort/turn tuning to the specifier spawn automatically.
...
**Delegate to**: `specifier` agent — spawn plainly; the `phase-tuning` hook applies `sdlc.yml`'s `phases.specifier` tuning at the spawn boundary (frontmatter default stands when unset).
