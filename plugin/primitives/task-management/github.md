...
| `set-blocking(blocked_by, blocks)` | **Native dependency** (first-class "Blocked by" relation + "Blocked" board icon). Resolve global node IDs (`gh issue view <n> --json id`), then GraphQL `addBlockedBy(input:{issueId:<blocks>, blockingIssueId:<blocked_by>})` via `gh api graphql` — `<blocks>` becomes blocked by `<blocked_by>`. `gh` exposes no CLI flag for this (as of 2.82), so it must go through GraphQL. **Idempotent**: query `issue.blockedBy(first:100){nodes{number}}` first and skip pairs already set. **Fallback** (older repo / missing scope / mutation unavailable): append a `Depends on: #<blocked_by>` body line — a cross-link mention only, not a true relation. Implemented in `plugin/scripts/sync-github.py` (`add_blocked_by` / `blocked_by_numbers`). |
