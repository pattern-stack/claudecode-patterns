...
# Launch Claude Code with this repo's plugin loaded directly from the working
# tree (no install, no symlinks). Edits to plugin/ files surface live; run
# `/reload-plugins` inside the session to pick them up without restart.
# Per claude-platform docs: --plugin-dir overrides any same-name marketplace
# install for the session, so dogfooding here doesn't conflict with users who
# installed `sdlc` via `/plugin install`.
dev:
