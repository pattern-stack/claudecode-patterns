---
description: Author / tune / validate / reverse-engineer / explain a canvas (a template + instructions pair under .claude/artifacts/<name>/) via the canvas-author agent. Probing schema-aware dialog; never writes without confirmation. v1 ships tune + validate.
...
> **What's a canvas?** A `template.md` + `instructions.yaml` + `instructions.schema.json` + `README.md` quartet under `.claude/artifacts/<name>/` defining the contract for one kind of artifact. See [`artifacts/README.md`](../artifacts/README.md).

## Pre-rendered context

Existing canvases:
!`ls -1 .claude/artifacts/*/README.md 2>/dev/null | sed 's|.claude/artifacts/||; s|/README.md||' | sort`
...
`<name>` matches a directory under `.claude/artifacts/` (or, for `new` / `reverse`, the directory to create).
...
- For `tune | validate | explain`: canvas must exist at `.claude/artifacts/<name>/`. If not, halt with the list of existing canvases.
- For `new | reverse`: canvas must NOT exist at `.claude/artifacts/<name>/`. If it does, suggest `tune` instead.
...
- `.claude/artifacts/<name>/README.md` (if exists) — what the canvas is
- `.claude/artifacts/<name>/instructions.yaml` (if exists) — current knob values
- `.claude/artifacts/<name>/instructions.schema.json` (if exists) — type contract
- `.claude/artifacts/<name>/template.md` (if exists) — current structure
...
- **Output**: file writes to `.claude/artifacts/<name>/...` on apply; session summary on close
...
| Before adding a new canvas to the registry | Out-of-band | After `new` or `reverse` mode closes, user adds `<name>:` to `sdlc.yml.artifacts` themselves |
...
For `tune` / `new` / `reverse`: file writes to `.claude/artifacts/<name>/`. The session summary at close lists what was applied.
